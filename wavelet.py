...
fig = make_subplots(rows=1, cols=1)
fig.add_trace(go.Scatter(x=xf, y=yf), row=1, col=1)
fig.update_layout(height=400, width=800, title_text="смещение частоты на fc-fp")
fig.show()
'''


'''
y = demod(Uc, fc, Um, fm, m, 8, L)
t = 5
N = len(y)
yf = np.abs(np.fft.fft(y))
xf = np.fft.fftfreq(N) * N * 1 / t
fig = make_subplots(rows=1, cols=1)
fig.add_trace(go.Scatter(x=xf, y=yf), row=1, col=1)
fig.update_layout(height=400, width=800, title_text="y")
fig.show()
'''